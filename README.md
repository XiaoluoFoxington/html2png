...
- **左右分栏（各 50%）**：左栏为「代码 / 配置 / 关于」三个标签页——代码编辑、
  输出参数与快捷键/使用方法/开源信息分区放置；右栏为实时预览
- **响应式**：宽屏左右 50% 分栏；平板及以下（≤980px）预览在上、
  标签页在下、各占剩余高度 50%（固定视口无滚动）；手机（≤768px）
  顶栏自动换行、标签均分整行、间距紧凑；标尺在所有宽度均显示
...
- **用户预设**：把当前代码框中的 HTML 命名保存为预设（存于 localStorage），
  之后从顶栏下拉框一键加载；编辑已加载的预设后点「保存预设」会自动预填
  当前预设名，一键覆盖保存；支持删除
...
左栏标签页切换 / 预设保存-加载-删除 / 自动刷新与手动刷新 / 清空 /
输入 / 持久化 / Ctrl+Enter 下载 / 手机视口响应式）：
